    ...
    def _load_from_db(self, tenant_id: str) -> list[Flow]:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                # Load flows (columns per infra/migrations/002_create_flows.sql)
                cur.execute(
                    """
        ...
        for row in node_rows:
            ...
            # Nodes are keyed by node_key: flows.entry_node and every
            # transition `next` reference node_key, never the row UUID.
