...
user flow_engine_user on >${REDIS_FLOW_ENGINE_PASSWORD} +xreadgroup +xack +xpending +xclaim +xadd +xgroup +xautoclaim +keys +hgetall +hset +expire +set +get +del +incr +exists +ping ~session:* ~flow-engine:* ~lock:flow:* ~rate:tenant:* ~processed:* ~tokens:tenant:* ~dry-session:*
